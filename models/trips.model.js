...
const editById = ({ min_traveler, max_traveler, min_age, max_age, departure_date, duration, price, description, flights, hotel, meals, excursions, rent_car, insurance }, tripId) => {
    return db.query('UPDATE trips SET min_traveler = ?, max_traveler = ?, min_age = ?, max_age = ?, departure_date = ?, duration = ?, price = ?, description = ?, flights = ?, hotel = ?, meals = ?, excursions = ?, rent_car = ?, insurance = ? WHERE id = ?;', [min_traveler, max_traveler, min_age, max_age, departure_date, duration, price, description, flights, hotel, meals, excursions, rent_car, insurance, tripId]);
}
...
const getAllDestinations = (destination) => {
    return db.query('SELECT destination AS name FROM trip2gether.trips WHERE status = "notstarted"', [destination]);
}

const getTripsSuscribedByUser = (userId) => {
    return db.query('SELECT t.* FROM users_has_trips ut JOIN trips t ON t.id = ut.trips_id WHERE ut.users_id = ? AND ut.user_status = "aceptada"', [userId]);
}

const getTripsCreatedByUser = (userId) => {
    return db.query('SELECT * FROM trips WHERE user_id = ? AND status <> "deleted"', [userId]);
}
...
const getOnlyPendingByTrip = (tripId) => {
    return db.query('SELECT * FROM users_has_trips uht JOIN users u ON uht.users_id = u.id WHERE trips_id = ? AND user_status = "pendiente"', [tripId])
}
...
module.exports = { getAll, create, editById, deleteById, getTripById, getTripsByDestination, getTripsCreatedByUser, createComment, getCommentsByTrips, getAllDestinations, getTripsSuscribedByUser, createItinerary, createRequest, getItineraryByTrip, getSubscribedByTrip, manageRequest, getGeometry, getUsersAccepted, getOnlyPendingByTrip }
